	...
	public static User setUpdateUser(User user, @Valid UpdateUserRequest request) {
		...
		if("YES".equalsIgnoreCase(request.getHomeAddress())) {
			Address homeAddress = addresses.stream()
					.filter(addr -> "Heme address".equalsIgnoreCase(addr.getType()))
					.findFirst()
					.orElse(new Address());
			...
			if(homeAddress.getId() <= 0)
				homeAddress.setCreated(DateUtil.getCurrentDate());
			...
			homeAddress.setUpdated(DateUtil.getCurrentDate());
			addresses.add(homeAddress);
		}
		
		if("YES".equalsIgnoreCase(request.getWorkAddress())) {
			
			Address workAddress = addresses.stream()
					.filter(addr -> "Work address".equalsIgnoreCase(addr.getType()))
					.findFirst()
					.orElse(new Address());
			
			if(workAddress.getId() <= 0)
				workAddress.setCreated(DateUtil.getCurrentDate());
			...
			workAddress.setUpdated(DateUtil.getCurrentDate());
